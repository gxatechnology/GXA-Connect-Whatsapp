...
export interface CampaignBatchSummary extends BatchStatusResponse {
...
export const messageApi = {
  ...
  listBatches: (sessionId: string, limit = 100) =>
    request<CampaignBatchSummary[]>(`/sessions/${sessionId}/messages/batches?limit=${limit}`),
...
// =============================================================================
// User & Auth API (Multi-User Role Management)
// =============================================================================
...
export const usersApi = {
  ...
  update: (id: string, body: { fullName?: string; role?: string; status?: string }) =>
...
export const authApi = {
  ...
  me: () => request<{ user?: User; role: string; authType: 'user' | 'api-key' }>('/auth/me'),
  changePassword: (body: { currentPassword: string; newPassword: string }) =>
    request<{ success: boolean }>('/auth/change-password', { method: 'POST', body: JSON.stringify(body) }),
};

// =============================================================================
// CRM API (Leads, Tags, Notes, Follow-ups, Activity, Campaign History)
// =============================================================================
...
export interface CrmCampaignHistoryItem {
  batchId: string;
  campaignName: string;
  sessionId: string;
  status: string;
...
export const crmApi = {
  getLeads: (query?: {
    ...
    return request<{ leads: CrmLead[]; total: number }>(`/crm/leads${qs ? `?${qs}` : ''}`);
  },
  getLead: (id: string) => request<CrmLead>(`/crm/leads/${id}`),
  getLeadByPhone: (phone: string, sessionId?: string) => {
    const params = new URLSearchParams({ phone });
    if (sessionId) params.set('sessionId', sessionId);
    return request<CrmLead | null>(`/crm/leads/by-phone?${params.toString()}`);
  },
  createLead: (body: {
    name: string;
    phone: string;
  ...
  updateLead: (id: string, body: Partial<CrmLead>) =>
    request<CrmLead>(`/crm/leads/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(body),
    }),
  deleteLead: (id: string) =>
    request<{ success: boolean }>(`/crm/leads/${id}`, { method: 'DELETE' }),

  getTags: () => request<CrmTag[]>('/crm/tags'),
  createTag: (body: { name: string; color?: string }) =>
    request<CrmTag>('/crm/tags', { method: 'POST', body: JSON.stringify(body) }),
  deleteTag: (id: string) =>
    request<{ success: boolean }>(`/crm/tags/${id}`, { method: 'DELETE' }),
  addTagToLead: (leadId: string, tagId: string) =>
    request<{ success: boolean }>(`/crm/leads/${leadId}/tags`, {
      method: 'POST',
      body: JSON.stringify({ tagId }),
    }),
  removeTagFromLead: (leadId: string, tagId: string) =>
    request<{ success: boolean }>(`/crm/leads/${leadId}/tags/${tagId}`, { method: 'DELETE' }),

  getNotes: (leadId: string) => request<CrmNote[]>(`/crm/leads/${leadId}/notes`),
  createNote: (leadId: string, content: string) =>
    request<CrmNote>(`/crm/leads/${leadId}/notes`, {
      method: 'POST',
      body: JSON.stringify({ content }),
    }),
  deleteNote: (id: string) =>
    request<{ success: boolean }>(`/crm/notes/${id}`, { method: 'DELETE' }),
  ...
  getFollowups: (query?: {
    ...
    if (query?.offset) params.set('offset', String(query.offset));
    const qs = params.toString();
    return request<{ followups: CrmFollowup[]; total: number }>(`/crm/followups${qs ? `?${qs}` : ''}`);
  },
  createFollowup: (body: {
    leadId: string;
    title: string;
    ...
    request<CrmFollowup>('/crm/followups', {
      method: 'POST',
      body: JSON.stringify(body),
    }),
  updateFollowup: (id: string, body: Partial<CrmFollowup>) =>
    request<CrmFollowup>(`/crm/followups/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(body),
    }),
  deleteFollowup: (id: string) =>
    request<{ success: boolean }>(`/crm/followups/${id}`, { method: 'DELETE' }),

  getActivity: (leadId: string) => request<CrmActivity[]>(`/crm/leads/${leadId}/activity`),
  getCampaignHistory: (leadId: string) =>
    request<CrmCampaignHistoryItem[]>(`/crm/leads/${leadId}/campaign-history`),
};
