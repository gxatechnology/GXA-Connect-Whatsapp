...
import { CreateApiKeyDto, UpdateApiKeyDto, LoginDto, LoginResponseDto } from './dto';
...
import { globalLoginRateLimiter } from './login-rate-limiter';
...
export class AuthService implements OnModuleInit, OnModuleDestroy {
  ...
  async createApiKey(dto: CreateApiKeyDto): Promise<{ apiKey: ApiKey; rawKey: string }> {
    ...
    const apiKey = this.apiKeyRepository.create({
      ...
      organizationId: dto.organizationId || 'org-platform-default',
  ...
  async loginUser(dto: LoginDto, clientIp?: string): Promise<LoginResponseDto> {
    const email = dto.email.toLowerCase().trim();
    const rateLimitKey = clientIp ? `${clientIp}:${email}` : email;

    // Check brute-force rate limit
    globalLoginRateLimiter.check(email);
    if (clientIp) {
      globalLoginRateLimiter.check(clientIp);
    }
    ...
    if (!user || user.status === UserStatus.DISABLED) {
      globalLoginRateLimiter.recordFailure(email);
      if (clientIp) globalLoginRateLimiter.recordFailure(clientIp);
      throw new UnauthorizedException('Invalid email or password');
    }

    const validPassword = verifyPassword(dto.password, user.passwordHash);
    if (!validPassword) {
      globalLoginRateLimiter.recordFailure(email);
      if (clientIp) globalLoginRateLimiter.recordFailure(clientIp);
      throw new UnauthorizedException('Invalid email or password');
    ...
    globalLoginRateLimiter.reset(email);
    if (clientIp) globalLoginRateLimiter.reset(clientIp);

    user.lastLoginAt = new Date();
    await this.userRepository.save(user);

    const token = this.mintUserToken(user);
    return {
      token,
      user: {
        id: user.id,
  ...
  mintUserToken(user: User): string {
    const payloadObj = {
      uid: user.id,
      email: user.email,
      role: user.role,
    ...
    const signature = createHmac('sha256', this.tokenSecret).update(payloadStr).digest('hex');
    return `owa_usr_${Buffer.from(payloadStr).toString('base64url')}.${signature}`;
  }

  async validateUserToken(rawToken: string): Promise<User> {
    if (!rawToken.startsWith('owa_usr_')) {
      throw new UnauthorizedException('Invalid token format');
    }

    const parts = rawToken.substring(8).split('.');
    if (parts.length !== 2) {
    ...
    const expectedSig = createHmac('sha256', this.tokenSecret).update(payloadStr).digest('hex');
    ...
    let payload: { uid: string; exp: number; tokenVersion?: number };
    try {
    ...
    // Server-side session revocation: verify tokenVersion matches current user record
    if (
  ...
  hasPermission(actor: ApiKey | User | { role: string }, requiredRole: ApiKeyRole | UserRole | string): boolean {
    ...
    const actorRole = (actor.role || 'viewer').toLowerCase();
    const reqRole = (typeof requiredRole === 'string' ? requiredRole : String(requiredRole)).toLowerCase();

    const actorLvl = roleHierarchy[actorRole] ?? 1;
    const reqLvl = roleHierarchy[reqRole] ?? 1;

    return actorLvl >= reqLvl;
  }
}
