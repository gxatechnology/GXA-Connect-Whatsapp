import { Controller, Post, Get, Param, Body, Query, Res, Req, HttpCode, HttpStatus, StreamableFile } from '@nestjs/common';
...
export class MessageController {
  ...
  async sendBulk(
    ...
    const orgId = (req as Request & { organizationId?: string }).organizationId || 'org-platform-default';
    const batch = await this.bulkMessageService.createBatch(sessionId, dto, orgId);
  ...
  @Get('batches')
  @ApiOperation({ summary: 'List real bulk-message campaigns for a session' })
  @ApiParam({ name: 'sessionId', description: 'Session ID' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Max campaigns to return (default 100, max 250)' })
  async listBatches(@Param('sessionId') sessionId: string, @Query('limit') limit?: string) {
    const batches = await this.bulkMessageService.listBatches(sessionId, limit ? parseInt(limit, 10) : 100);
